...
bool can_castle_kingside(const Piece &king, const Position &king_position, const Board &board) {
    ...
    if (king.has_moved || king.type != PieceType::KING) {
        return false;
    }

    // Get the rook's position for kingside
    Position rook_position = {king_position.row, BOARD_SIZE - 1};
    auto rook = board.get_piece(rook_position.row, rook_position.col);

    if (rook.type != PieceType::ROOK || rook.color != king.color || rook.has_moved) {
        return false;
    }
...
bool can_castle_queenside(const Piece &king, const Position &king_position, const Board &board) {
    ...
    Position rook_position = {king_position.row, 0};
    auto rook = board.get_piece(rook_position.row, rook_position.col);
